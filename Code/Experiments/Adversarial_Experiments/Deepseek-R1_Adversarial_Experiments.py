...
def extract_boxed_text(text):
    ...
    matches = re.findall(r'</think>\s*(.*)', text, re.DOTALL)
    return matches[-1].strip() if matches else "pass"
...
def metrics(a, domain, list1, list5):
    ...
    with open(f"Adversarial_DeepSeek_Results_{domain}.txt",'a') as f:
...
def func_none(r):
    return r if r is not None else "pass"
        ...
        for domain, papers in data.items():
            for paper_url, info in papers.items():
                ...
                for sentence_info in sentences:
                    ...
                    r = generate_response(f"I have taken a sentence from the research paper titled \"{most_similar_title}\", give me the title of the possible research paper that this sentence is citing to. If you are not able to come up with the paper title write 'pass'. Don't write anything else. \"{user_content}\"")
                    r = func_none(r)
                    r = extract_boxed_text(r)
                    list2.append(r)
                    if r.lower()=='pass' or compare(list2,list5):
                      ...
                      list1.append(r)
                    print(list1,"\n", list5)
        metrics("SID with paper title",d,list1,list5)
        list1.clear()
        list2.clear()
        list3.clear()
        list5.clear()
        ...
        for domain, papers in data.items():
            for paper_url, info in papers.items():
                ...
                for sentence_info in sentences:
                    ...
                    author = sentence_info["Citation"]["Citation Paper Authors"]
                    abstract = sentence_info["Citation"]["Citation Paper Abstract"]
                    other_abstracts = [sentence["Citation"]["Citation Paper Abstract"] for sentence in sentences if sentence["Citation"]["Citation Paper Abstract"] != abstract]
                    most_similar_abstract = find_most_similar_abstract(abstract, other_abstracts)

                    print(f"Paper Title: {paper_title}")
                    print(f"Original Abstract: {abstract}")
                    print(f"Most Similar Abstract: {most_similar_abstract}")
                    list5.append(paper.split(":", 1)[1].strip())
                    retrieved_docs = query_engine.query(user_content)
                    context = retrieved_docs.response[:500] if hasattr(retrieved_docs, 'response') else ""
                    r = generate_response(f"I have taken a sentence from the research paper titled \"{paper_title}\", give me the title of the possible research paper that this sentence is citing to. If you are not able to come up with the paper title write 'pass'. Don't write anything else. \"{user_content}\"")
                    r = func_none(r)
                    r = extract_boxed_text(r)
                    list2.append(r)
                    if r.lower()=='pass' or compare(list2,list5):
                      retrieved_docs = query_engine.query(user_content)
                      context = retrieved_docs.response[:500] if hasattr(retrieved_docs, 'response') else ""
                      r1 = generate_response(f"I have taken a sentence from the research paper titled \"{paper_title}\", give me the title of the possible research paper that this sentence is citing to. If you are not able to come up with the paper title write 'pass'. Don't write anything else. \"{user_content}\". Let me give you some more context by providing the abstract of the research paper it is citing to. {most_similar_abstract}")
                      r1 = func_none(r1)
                      r1 = extract_boxed_text(r1)
                      list3.append(r1)
                      if r1.lower()=='pass' or compare(list3,list5):
                      ...
                      list1.append(r)
                    print(list1,"\n", list5)
        metrics("SID with abstract",d,list1,list5)
        list1.clear()
        list2.clear()
        list3.clear()
        list5.clear()
...
def main():
  file_name = "" # Give the file name/file path over here
  domain = 'Mixed' # specify the domain over here
